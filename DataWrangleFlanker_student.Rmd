...
ERT<-CRT%>%filter=(as.numeric(CRT<=2000))

#CRT = Calculate Reaction Time
...
CMRT <- CRT%>%group_by(Trial_Con=as.numeric(AC=="1"))%>%summarise(mean(CRT))
...
OMSRTs <- CRT%>%group_by(Trial_Con=as.numeric(AC=="1"))%>%
  summarise(CMRT=mean(CRT), 
            SEM_CRT=(sd(AC=="1")/sqrt(length(AC=="1"))))


#OMSRTs = Overall Mean & SEM Reaction Times
#CMRT = Calculated Mean Reaction Time
#SEM_CRT = Standard Errors of Mean
#Trial_Con = Trial Congruency
#CRT = Calculate Reaction Time
#AC=Accuracy Column
#1=Congruency & 0=Incongruency
...
```{r}
Flanker_Effect <- OMSRTs%>%mutate(Flanker_Effect=CMRT[1,2]-CMRT[2,2])
