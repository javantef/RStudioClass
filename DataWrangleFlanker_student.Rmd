...
```{r}
#An accuracy column that codes for correct responses (AC=Accuracy Column)

AC<-one_subject%>%mutate(AC=as.numeric(congruency=="C"))

#1=Congruency & 0=Incongruency
```
...
```{r}
#The difference between the two (response_time  - stimulus_onset) is the reaction time in milliseconds (CRT= Calculated Reaction Time)

CRT<-AC%>%mutate(CRT=response_time - stimulus_onset)
...
TC<-all_data%>%group_by(CRT=response_time - stimulus_onset,AC=as.numeric(congruency=="C"),subject)%>%
            summarise(MAC=mean(AC),
            CMRT=mean(response_time - stimulus_onset), 
            TC=length(trial))
...
#AC=Accuracy Column
#MAC=Mean Accuracy Column
#CMRT=Calculated Mean Reaction Time
...
```{r}
#ERT=Excluded Reaction times

ERT<-all_data%>%filter(response_time,stimulus_onset)%>%
                group_by(ERT=response_time-stimulus_onset, ERT<2000)
...
#RTA = Reaction Time Analysis


RTA <- TC%>%filter(as.numeric(AC)=="1")%>%group_by(subject)%>%summarise(CMRT)


#TC=Trial Checks
#AC=Accuracy Column
#Trial_Con = Trial Congruency
#CMRT = Calculated Mean Reaction Time
#1=Congruency & 0=Incongruency
...
#OMSRTs = Overall Mean & SEM Reaction Times


OMSRTs <- TC%>%group_by(as.numeric(AC))%>%
  summarise(OM=mean(CMRT),SEM_CRT=(sd(as.numeric(AC))/sqrt(length(as.numeric(AC)))))



ggplot(OMSRTs, aes(x=as.numeric(AC),y=OM))+
  geom_bar(stat="identity")+
  geom_errorbar(aes(ymin=OM-SEM_CRT,
                    ymax=OM+SEM_CRT),
                width=.2)+
  coord_cartesian(ylim=c(0,10.0))+
  xlab("Congruency")+
  ylab("Calculated Mean Reaction Time")+
  theme_classic(base_size=12)

#TC=Trial Checks
#OM=Overall Mean
#CMRT = Calculated Mean Reaction Time
#SEM_CRT = Standard Errors of Mean for Calculate Reaction Time
#Trial_Con = Trial Congruency
#AC=Accuracy Column
#1=Congruency & 0=Incongruency
...
```{r}
Flanker_Effect <- OMSRTs%>%filter(OM,SEM_CRT)
 

#OM=Overall Mean
#DCMRT = Difference between Calculated Mean Reaction Times
#OMSRTs = Overall Mean & SEM Reaction Times
#CMRT = Calculated Mean Reaction Time
